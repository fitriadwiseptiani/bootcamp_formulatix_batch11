...
class Program
{
	static void Main()
	{
		using (ExampleDatabase exampleDatabase = new())
		{
			//Connection Test
			bool status = exampleDatabase.Database.CanConnect();
			if (status == false)
			{
				return;
			}
			...
			//Search/Filter
			List<Category> categories = exampleDatabase.Categories.Where(a => a.CategoryName.Contains("o")).Include(a => a.Products).ToList();
